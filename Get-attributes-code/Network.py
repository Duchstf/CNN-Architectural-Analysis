"""
Authors: Jesse Hamer (1), Duc Hoang - SIST intern, Summer 2019 (Rewrote documentation and wrote extended functions) (2) 

This is a collection of classes meant for handling caffe layer objects to extract 
...
There are two main classes of functions that do the following: 

- Extract "simple attributes," which only concern the architecture of the networks without considering the inputs.

- Extract "complex attributes," which analyze the input's topological features and "score it" or let it transforms with 
  regards to the whole structure of the networks. These are not generic for different networks files. 
"""

#Standard Data Handling and Analysis
import numpy as np
import matplotlib.pyplot as plt
from mpl_toolkits.mplot3d import Axes3D
from mpl_toolkits.mplot3d.art3d import Poly3DCollection, Line3DCollection

#Iteration tools, topological analysis, and image handling
import itertools
import gudhi as gd #Topological data analysis
import h5py #Handle HDF5 Data Format
import simplicial as simp

#Protobuf's file handling
...
# For those who have access to the Wilson Cluster, this directory is under /home/jhamer
...
class Network:
    ...
    - mode     -- str|one of 'minerva' or 'nova'; specifies how certain aspects of
    ...
    def __init__(self, caffeNet, mode='minerva'):
        ...
        with open(caffeNet) as f:
            text_format.Merge(f.read(), tmp_net) #Extract all net parameters from input prototxt file
        
        #Initilize some parameters of the networks
        self.name = tmp_net.name
        self._img_counter = 0
        self.layers = {} #Dictionary to save each layer's parameters
        self.dataset = mode
        
        # Maintain special list of input layers for quick lookup
        self.inputLayers = []
        trash_list = [] #DUC: Don't really know the purpose of this but might be layers to ignore. 
        
        for layer in tmp_net.layer: #Each layer is a Caffe layer protobuf message
            ...
            self.layers[layer_name] = Layer(caffeLayer=layer) #Pass the key (an layer object) to dictionary
        ...
        if mode == 'minerva':
            ...
            if name in ['data0_1', 'data0_2']:
                return {
                    ...
                    'height': input_h,
                    'width': input_w,
                    'include': list(caffeLayer.include)
                }
            else:
                return {
                    'name': name,
    ...
    def get_grids(input_layer, output_layer):
        ...
        op_grid_h = int(max([1 + (ip_layer_grid_h + 2 * pad_h - kernel_h) / stride_h, 1]))
        op_grid_w = int(max([1 + (ip_layer_grid_w + 2 * pad_w - kernel_w) / stride_w, 1]))

        coverage_h = kernel_h + (op_grid_h-1)*stride_h
        coverage_w = kernel_w + (op_grid_w-1)*stride_w

        #CAFFE TRUNCATES BOUNDARY PIXELS WHEN STRIDE IS NONZERO. THE FOLLOWING LINES CORRECT THIS.
        #IF THIS BEHAVIOR IS NOT DESIRED, COMMENT OUT THESE LINES.
        if coverage_h < pad_h + ip_layer_grid_h:
            op_grid_h += 1
        if coverage_w < pad_w + ip_layer_grid_w:
            op_grid_w += 1

        output_grid = np.zeros((op_channels, op_grid_h, op_grid_w))
        return input_grid, output_grid



#------COMPLEX ATTRIBUTES RELATED FUNCTION------#
    ...
    def get_flow(self, A, inputLayer, layerPath=None, D=0):
        ...
        if layerPath:
            ...
            for i in range(len(layerPath) - 1):
                ...
                if not activations:
                    print('List of activations in network {}, layer {} is empty.'.format(self.name, bottom_layer.name))
                    return [[None, None]]
    ...
    def _flow_once(A, bottom, top):
        ...
        top_r = top_layer.layerParams['output_grid'][0].shape[0]
        top_c = top_layer.layerParams['output_grid'][0].shape[1]
        ...
        for point in A:
            ...
            i2 = min([int(np.floor((i + pad_h) / stride_h)), top_r-1])
            j2 = min([int(np.floor((j + pad_w) / stride_w)), top_c-1])
    ...
    def get_layer_depth(self, layer_name, weightsOnly=False, phases=None,
        ...
        #Now that the input is found, start reconstructing path to input
    ...
    def ratio_features_to_depth(self, key='MAX', phases=None,
        ...
        for layer in layers:
            #If the layer's type is not concat and not a part of inception module add to remove list
            if self.layers[layer].type == 'Concat' and 'inception' not in layer:
                remove_list.append(layer)
                continue
            
            #if don't want to include 1x1 kernel and is convolution
            ...
            #if inception unit is treated as a single layer is true and layer type is not concat add to remove list
            if inception_unit and 'inception' in layer and self.layers[layer].type != 'Concat':
                remove_list.append(layer)
        
        #Take all the layers that are not in the remove_list
        layers = [l for l in layers if l not in remove_list]
        
        #To save statisitcs summary of these layers
        stats = []

        for l in layers:
            
            #For each layer gets the layer's depth
            d = self.get_layer_depth(l, weightsOnly=True, phases=phases,
                                     inception_unit=inception_unit,
                                     include_pooling=include_pooling)[0]
                                     
            #features are number of output parameters in that layer
    ...
    def ratio_kerDim_to_depth(self, key='MAX', phases=None,
                              ...
                              dim='a'):
        """
            Uses key to return a statistical summary of layer's ratios of kernel's dimensionality to depth. Only layers with weights are considered. By default, inception modules are treated as single layers and inner product layers are considered.
    ...
    def get_img_points2d(self, inputLayer):
        ...
        """
        img = self.layers[inputLayer].layerParams['output_grid'][0]
        row, col = img.shape
        indices = itertools.product(range(row), range(col))

        return [(i, j) for i, j in indices if img[i, j] != np.float(0)]
    ...
    def get_img_alpha_cplx2d(self, inputLayer, alpha2='inf'):
        ...
        indices = self.layers[inputLayer].imgFeatures['points_2d']
    ...
    def set_img_alpha_cplx2d(self, inputLayer, alpha2='inf'):
        ...
        self.layers[inputLayer].imgFeatures['alpha_complex2dPers'] = self.layers[inputLayer].imgFeatures['alpha_complex2d'].persistence(2)
    ...
    def get_betti_numbers(self, inputLayer):
        """
        ...
        return self.layers[inputLayer].imgFeatures['alpha_complex2d'].persistent_betti_numbers(alpha2, alpha2)

    def get_num_persistent_components(self, inputLayer, pers_scaler=0.25):
        """
        ...
        mp = self.layers[inputLayer].imgFeatures['min_connected_alpha22d']

        pers = self.layers[inputLayer].imgFeatures['alpha_complex2dPers']
        zero_pers = [p[1] for p in pers if p[0] == 0]
        num = len([p for p in zero_pers if p[1] - p[0] > pers_scaler * mp])

        return num

    def get_num_persistent_holes(self, inputLayer, pers_scaler=0.25):
        """
        ...
        num = len([p for p in one_pers if p[1] - p[0] > pers_scaler * mp])
    ...
    def get_alpha_edges(self, inputLayer):
        ...
        skel1 = self.layers[inputLayer].imgFeatures['alpha_complex2d'].get_skeleton(1)
        alpha2 = self.layers[inputLayer].imgFeatures['min_connected_alpha22d']
        return len([p for p in skel1 if p[1] >= alpha2])-len(points)
    ...
    def inter_layer_bottleneck_avg(self):
        ...
        for ip1,ip2 in itertools.combinations(self.inputLayers, 2):
            pers1 = self.layers[ip1].imgFeatures['alpha_complex2dPers']
            pers2 = self.layers[ip2].imgFeatures['alpha_complex2dPers']

            diag1 = [p[1] for p in pers1]
            diag2 = [p[1] for p in pers2]
    ...
    #------SCORING FUNCTIONS - COMPLEX ATTRIBUTES RELATED------#

    def bottleneck_scores(self, layer_path):
        """
        Returns a 3-tuple of scores involving the bottlenecks between alpha-complexes in consecutive layers of
        layer_path.

        :param layer_path: a list of strings which are layer names
        :return: tuple of floats; (first_last_bottleneck, avg_consecutive_bottleneck, total_bottleneck_variation)
        """
        ...
        start_pers = ip_alpha.persistence(2)

        bottlenecks = []

        for op_layer in layer_path[1:]:
            flow = self.get_flow(A, ip_layer, [ip_layer, op_layer])[0][1]
            if flow is None or flow == []:
                print("ip_layer: {}, op_layer: {}. Exiting".format(ip_layer, op_layer))
            ...
            k_a = k_w * k_h
            num_samples = int((op_area / ip_area) * len(flow) / k_a)
            if num_samples <= 0:
                num_samples = 1
        ...
        end_pers = gd.AlphaComplex(points=A).create_simplex_tree().persistence(2)
        start_diag = [p[1] for p in start_pers]
        end_diag = [p[1] for p in end_pers]
        ...
        return gd.bottleneck_distance(start_diag, end_diag), np.mean(bottlenecks), sum(changes)
    ...
    def first_layer_scores(self, ip_layer):
        """
        Returns a tuple of scores based on the first layer following an input layer.
        ...
        zero_pers = [p[1] for p in op_pers if p[0] == 0]
        if not zero_pers or not ip_diag or not op_diag:
            return np.NaN, np.NaN
        if len(zero_pers) == 1:
            min_alpha2 = 0
        else:
            min_alpha2 = max([p[1] for p in zero_pers if p[1] < float('Inf')]) + 0.25

        return gd.bottleneck_distance(ip_diag, op_diag), min_alpha2
...
class Layer:
    ...
    Arguments:
    - caffeLayer -- a Caffe layer protobuf message
    - inputLayer -- a dict constructed by _getInputInfo
    - dummy      -- bool, indicates whether the layer is referenced in the caffeLayer.top attribute
    ...
    def __init__(self, caffeLayer=None, inputLayer=None, dummy=False):
        ...
        if not dummy:
            ...
            if caffeLayer:
                ...
                self.bottom = list(caffeLayer.bottom)
                self.top = []
                self.imgFeatures = {}
    ...
    def _getSliceParams(caffeLayer):
        return {
            ...
            'slice_point': list(caffeLayer.slice_param.slice_point)
        }
    ...
    def _getDummyDataParams(caffeLayer):
        if caffeLayer.dummy_data_param.shape:
            ...
            num = list(caffeLayer.dummy_data_param.num)
            channels = list(caffeLayer.dummy_data_param.channels)
            height = list(caffeLayer.dummy_data_param.height)
            width = list(caffeLayer.dummy_data_param.width)
        return {
    ...
    def _getConvolutionParams(self, caffeLayer):
        ...
        return {
            ...
            'dilation': list(caffeLayer.convolution_param.dilation),
            'pad_h': pad_h,
            'pad_w': pad_w,
            'kernel_h': kernel_h,
            'kernel_w': kernel_w,
    ...
    def _getSigmoidCrossEntropyLossParams(caffeLayer):
        return {
            'loss_weight': list(caffeLayer.loss_weight)
        }
    ...
    def _getEltwiseParams(caffeLayer):
        ...
        return {
            'operation': op,
            'coeff': list(caffeLayer.eltwise_param.coeff)
        }
