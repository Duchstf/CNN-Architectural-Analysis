...
def scrape_output(path, network_name, mode):
    ...
    #May, 30, 2019: Adjusted accuracy readout for MINERVA Networks, since the new files comes in in a different format
    if mode == 'minerva':
        output = path + '/' + 'output_' + network_name + '.txt'
        acc = ()
        with open(output) as op:
            for l in op.readlines()[len(op.readlines())-2:]: #op.readlines(): #Adjusted this line to similar line in NoVa
                try:
                    a = float(l[l.index('=') + 1:])
                except ValueError:
                    a = np.NaN
